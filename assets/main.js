...
function normalizedTwistIn(caliberId, value) {
    return positiveNumber(value, defaultTwistIn(caliberId));
}
function normalizedBulletLengthIn(load, value) {
...
const state = {
    ...
    targetOptionId: safeGetStorage("targetOptionId") === "no" || safeGetStorage("targetOptionId") === "half" || safeGetStorage("targetOptionId") === "full" ? safeGetStorage("targetOptionId") : "full",
...
function renderMovingTargetControls() {
...
function renderMovingTargetTable(calculations) {
    const targetOption = activeTargetOption();
    if (state.targetSpeedMs <= 0 || targetOption.factor <= 0)
        return "";
    const marks = distanceMarks();
    return `
...
function bindEvents() {
    ...
    document.querySelectorAll("[data-target-option]").forEach(button => {
        button.addEventListener("click", () => setState({ targetOptionId: button.dataset.targetOption ?? state.targetOptionId }));
    });
    ...
    targetSpeedInput?.addEventListener("change", () => {
        ...
        if (Number.isFinite(value))
            setState({ targetSpeedMs: Math.max(0, state.unitSystem === "imperial" ? mphToMs(value) : value) });
    });
