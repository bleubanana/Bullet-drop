...
export function calculateMillerStability(input) {
    if (input.bulletWeightGr <= 0)
        throw new Error("Bullet weight must be positive");
    if (input.bulletDiameterIn <= 0)
        throw new Error("Bullet diameter must be positive");
    if (input.bulletLengthIn <= 0)
        throw new Error("Bullet length must be positive");
    if (input.twistInPerTurn <= 0)
        throw new Error("Twist must be positive");
