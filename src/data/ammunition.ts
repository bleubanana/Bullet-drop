...
export const AMMUNITION: AmmunitionLoad[] = [
  {
...
export function ammunitionById(id: string): AmmunitionLoad {
  const load = AMMUNITION.find(item => item.id === id);
  if (!load) throw new Error(`Unknown ammunition load: ${id}`);
  return load;
}

export function ammunitionForCaliber(caliberId: string): AmmunitionLoad[] {
