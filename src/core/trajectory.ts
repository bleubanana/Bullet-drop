...
export function solveRawTrajectory(input: SolverInput): RawTrajectory[] {
  ...
  if (input.ballisticCoefficientG1 <= 0) throw new Error("Ballistic coefficient must be positive");
  ...
  while (x <= maxDistance + 1 && distanceIndex < distances.length) {
    ...
    if (targetDistance === undefined) break;
...
export function lineOfSightDropCm(raw: RawTrajectory[], distanceM: number, zeroM: number, sightHeightCm: number): number {
  const point = raw.find(item => item.distanceM === distanceM);
  const zeroPoint = raw.find(item => item.distanceM === zeroM);
  if (!point) throw new Error(`Missing trajectory point for ${distanceM} m`);
  if (!zeroPoint) throw new Error(`Missing zero trajectory point for ${zeroM} m`);
...
export function windDriftCm(timeOfFlightS: number, distanceM: number, muzzleVelocityMs: number, windSpeedMs: number, windValueFactor: number): number {
  if (windSpeedMs <= 0 || windValueFactor <= 0) return 0;
  const vacuumTime = distanceM / muzzleVelocityMs;
  return windSpeedMs * windValueFactor * Math.max(0, timeOfFlightS - vacuumTime) * 100;
}

export function calculateTrajectory(input: CalculationInput): TrajectoryPoint[] {
  ...
  return input.distancesM.map(distanceM => {
    ...
    if (!rawPoint) throw new Error(`Missing trajectory point for ${distanceM} m`);
